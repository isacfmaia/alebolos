  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          'fixed top-1/2 left-1/2 z-50 w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl',
        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            render={
              <Button
                variant="ghost"
                className="text-muted-foreground hover:text-brand-brown absolute top-3 right-3"
                size="icon-sm"
              />
            }
          >
...
function DialogTitle({ className, ...props }: DialogPrimitive.Title.Props) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn('font-heading text-brand-brown text-xl font-bold', className)}
      {...props}
    />
  )
}
